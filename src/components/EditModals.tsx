...
interface VE { [k: string]: string }

export function EditPersonalInfoModal({ isOpen, onClose, onSave, data }: { isOpen: boolean; onClose: () => void; onSave: (d: PersonalInfo) => void; data: PersonalInfo }) {
  const [form, setForm] = useState<PersonalInfo>(data);
  const [errors, setErrors] = useState<VE>({});
  const photoInputRef = useRef<HTMLInputElement>(null);
  const { t } = useLanguage();
  useEffect(() => { if (isOpen) { setForm(data); setErrors({}); } }, [isOpen, data]);
  const validate = (): boolean => { const e: VE = {}; if (!validateRequired(form.name)) e.name = t('editPersonalInfo.nameRequired'); if (!validateRequired(form.title)) e.title = t('editPersonalInfo.titleRequired'); if (!validateRequired(form.email)) e.email = t('editPersonalInfo.emailRequired'); else if (!validateEmail(form.email)) e.email = t('editPersonalInfo.emailInvalid'); if (!validateRequired(form.phone)) e.phone = t('editPersonalInfo.phoneRequired'); else if (!validatePhone(form.phone)) e.phone = t('editPersonalInfo.phoneInvalid'); if (!validateRequired(form.address)) e.address = t('editPersonalInfo.addressRequired'); setErrors(e); return !Object.keys(e).length; };
  const handleChange = (f: keyof PersonalInfo, v: string) => { setForm(p => ({ ...p, [f]: v })); if (errors[f]) setErrors(p => ({ ...p, [f]: '' })); };
  const handlePhoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => setForm(p => ({ ...p, photoUrl: event.target?.result as string }));
...
export function EditExperienceModal({ isOpen, onClose, onSave, data }: { isOpen: boolean; onClose: () => void; onSave: (d: Experience) => void; data: Experience | null }) {
  const empty = { id: generateId(), company: '', position: '', startDate: '', endDate: '', description: '' };
  const [form, setForm] = useState<Experience>(data || empty);
  const [errors, setErrors] = useState<VE>({});
  const { t } = useLanguage();
  useEffect(() => { if (isOpen) { setForm(data || { ...empty, id: generateId() }); setErrors({}); } }, [isOpen, data]);
  const validate = (): boolean => { const e: VE = {}; if (!validateRequired(form.company)) e.company = t('editExperience.companyRequired'); if (!validateRequired(form.position)) e.position = t('editExperience.positionRequired'); if (!validateRequired(form.startDate)) e.startDate = t('editExperience.startDateRequired'); if (!validateRequired(form.description)) e.description = t('editExperience.descriptionRequired'); setErrors(e); return !Object.keys(e).length; };
  const handleChange = (f: keyof Experience, v: string) => { setForm(p => ({ ...p, [f]: v })); if (errors[f]) setErrors(p => ({ ...p, [f]: '' })); };
  const inp = (label: string, field: keyof Experience, ph: string, dir?: string, type = 'text') => (<div><label className="mb-1.5 block text-sm font-medium text-text-secondary">{label}</label><input type={type} value={form[field]} onChange={e => handleChange(field, e.target.value)} className={`w-full rounded-xl border bg-bg-input py-2.5 px-4 text-sm text-text-primary placeholder:text-text-secondary/40 transition-all focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/30 ${errors[field] ? 'border-red-400' : 'border-border-gold'}`} placeholder={ph} dir={dir} />{errors[field] && <p className="mt-1 flex items-center gap-1 text-xs text-red-400"><AlertCircle size={12} />{errors[field]}</p>}</div>);
  return (<Modal isOpen={isOpen} onClose={onClose} title={data ? t('editExperience.titleEdit') : t('editExperience.titleAdd')} size="lg"><div className="space-y-4">{inp(t('editExperience.position'), 'position', t('editExperience.positionPlaceholder'))}{inp(t('editExperience.company'), 'company', t('editExperience.companyPlaceholder'))}<div className="grid grid-cols-2 gap-3">{inp(t('editExperience.startDate'), 'startDate', '', 'ltr', 'month')}<div><label className="mb-1.5 block text-sm font-medium text-text-secondary">{t('editExperience.endDate')}</label><input type="text" value={form.endDate} onChange={e => handleChange('endDate', e.target.value)} className="w-full rounded-xl border border-border-gold bg-bg-input py-2.5 px-4 text-sm text-text-primary transition-all focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/30" placeholder={t('editExperience.endPlaceholder')} /></div></div><div><label className="mb-1.5 block text-sm font-medium text-text-secondary">{t('editExperience.description')}</label><textarea value={form.description} onChange={e => handleChange('description', e.target.value)} rows={4} className={`w-full resize-none rounded-xl border bg-bg-input py-2.5 px-4 text-sm text-text-primary placeholder:text-text-secondary/40 transition-all focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent/30 ${errors.description ? 'border-red-400' : 'border-border-gold'}`} placeholder={t('editExperience.descPlaceholder')} />{errors.description && <p className="mt-1 flex items-center gap-1 text-xs text-red-400"><AlertCircle size={12} />{errors.description}</p>}</div><div className="flex gap-3 pt-2"><button onClick={() => { if (validate()) { onSave(form); onClose(); } }} className="flex-1 rounded-xl bg-gradient-to-l from-accent to-accent-dark py-2.5 text-sm font-semibold text-bg-primary transition-all hover:shadow-lg hover:shadow-accent/20">{t('modal.save')}</button><button onClick={onClose} className="flex-1 rounded-xl border border-border-gold py-2.5 text-sm text-text-secondary transition-all hover:border-accent hover:text-accent">{t('modal.cancel')}</button></div></div></Modal>);
}

export function EditSkillsModal({ isOpen, onClose, onSave, skills }: { isOpen: boolean; onClose: () => void; onSave: (s: Skill[]) => void; skills: Skill[] }) {
  const [form, setForm] = useState<Skill[]>(skills);
  const { t } = useLanguage();
  useEffect(() => { if (isOpen) setForm([...skills]); }, [isOpen, skills]);
  return (<Modal isOpen={isOpen} onClose={onClose} title={t('editSkills.title')} size="lg"><div className="space-y-3">{form.map(s => (<div key={s.id} className="flex items-center gap-3 rounded-xl border border-border-gold bg-bg-primary/30 p-3"><input type="text" value={s.name} onChange={e => setForm(p => p.map(x => x.id === s.id ? { ...x, name: e.target.value } : x))} className="flex-1 rounded-lg border border-border-gold bg-bg-input py-2 px-3 text-sm text-text-primary placeholder:text-text-secondary/40 focus:border-accent focus:outline-none" placeholder={t('editSkills.skillPlaceholder')} /><div className="flex items-center gap-2"><input type="range" min="10" max="100" value={s.level} onChange={e => setForm(p => p.map(x => x.id === s.id ? { ...x, level: parseInt(e.target.value) } : x))} className="w-20 accent-accent" /><span className="w-10 text-center text-xs text-accent">{s.level}%</span></div><button onClick={() => setForm(p => p.filter(x => x.id !== s.id))} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-text-secondary transition-all hover:bg-red-500/10 hover:text-red-400"><X size={14} /></button></div>))}<button onClick={() => setForm(p => [...p, { id: generateId(), name: '', level: 50 }])} className="w-full rounded-xl border border-dashed border-accent/30 py-2.5 text-sm text-accent transition-all hover:border-accent hover:bg-accent/5">{t('editSkills.addSkill')}</button><div className="flex gap-3 pt-2"><button onClick={() => { onSave(form.filter(s => validateRequired(s.name))); onClose(); }} className="flex-1 rounded-xl bg-gradient-to-l from-accent to-accent-dark py-2.5 text-sm font-semibold text-bg-primary transition-all hover:shadow-lg hover:shadow-accent/20">{t('modal.save')}</button><button onClick={onClose} className="flex-1 rounded-xl border border-border-gold py-2.5 text-sm text-text-secondary transition-all hover:border-accent hover:text-accent">{t('modal.cancel')}</button></div></div></Modal>);
}

export function EditLanguagesModal({ isOpen, onClose, onSave, languages }: { isOpen: boolean; onClose: () => void; onSave: (l: Language[]) => void; languages: Language[] }) {
  const [form, setForm] = useState<Language[]>(languages);
  const { t, levelOptions } = useLanguage();
  useEffect(() => { if (isOpen) setForm([...languages]); }, [isOpen, languages]);
  return (<Modal isOpen={isOpen} onClose={onClose} title={t('editLanguages.title')}><div className="space-y-3">{form.map(l => (<div key={l.id} className="flex items-center gap-3 rounded-xl border border-border-gold bg-bg-primary/30 p-3"><input type="text" value={l.name} onChange={e => setForm(p => p.map(x => x.id === l.id ? { ...x, name: e.target.value } : x))} className="flex-1 rounded-lg border border-border-gold bg-bg-input py-2 px-3 text-sm text-text-primary placeholder:text-text-secondary/40 focus:border-accent focus:outline-none" placeholder={t('editLanguages.langPlaceholder')} /><select value={migrateLevel(l.level)} onChange={e => setForm(p => p.map(x => x.id === l.id ? { ...x, level: e.target.value } : x))} className="rounded-lg border border-border-gold bg-bg-input py-2 px-3 text-sm text-text-primary focus:border-accent focus:outline-none">{levelOptions.map(o => <option key={o.key} value={o.key}>{o.label}</option>)}</select><button onClick={() => setForm(p => p.filter(x => x.id !== l.id))} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-text-secondary transition-all hover:bg-red-500/10 hover:text-red-400"><X size={14} /></button></div>))}<button onClick={() => setForm(p => [...p, { id: generateId(), name: '', level: 'intermediate' }])} className="w-full rounded-xl border border-dashed border-accent/30 py-2.5 text-sm text-accent transition-all hover:border-accent hover:bg-accent/5">{t('editLanguages.addLanguage')}</button><div className="flex gap-3 pt-2"><button onClick={() => { onSave(form.filter(l => validateRequired(l.name))); onClose(); }} className="flex-1 rounded-xl bg-gradient-to-l from-accent to-accent-dark py-2.5 text-sm font-semibold text-bg-primary transition-all hover:shadow-lg hover:shadow-accent/20">{t('modal.save')}</button><button onClick={onClose} className="flex-1 rounded-xl border border-border-gold py-2.5 text-sm text-text-secondary transition-all hover:border-accent hover:text-accent">{t('modal.cancel')}</button></div></div></Modal>);
}
...
export function EditCertificateModal({ isOpen, onClose, onSave, data }: { isOpen: boolean; onClose: () => void; onSave: (d: Certificate) => void; data: Certificate | null }) {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!f) return;
    e.target.value = '';
    if (f.size > 25 * 1024 * 1024) { setErrors(p => ({ ...p, file: t('editCertificate.fileTooLarge') })); return; }
    setUploading(true);
    setErrors(p => ({ ...p, file: '' }));
    try {
